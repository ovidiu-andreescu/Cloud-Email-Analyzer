...
def lambda_handler(event, context):
    ...
    logging.info(f"Mount path exists: {os.path.exists(os.environ['EFS_MOUNT_PATH'])}")
    logging.info(f"Mount path contents: {os.listdir(os.environ['EFS_MOUNT_PATH'])}")
    logging.info(subprocess.getoutput("which freshclam"))
    logging.info(subprocess.getoutput("freshclam --version"))
...
def freshclam_update(download_path):
    ...
    try:
        ...
        print(f"--- Freshclam Exit Code: {update_summary.returncode} ---")
        print(update_summary.stdout)
        ...
        report_failure(str(e.output) if e.output else str(e))
