...
def handler(event, context):
    ...
    MESSAGES.update_item(
        ...
        UpdateExpression="SET virusVerdict = :vv, finalVerdict = :fv, #st = :st",
        ExpressionAttributeNames={"#st": "status"},
        ExpressionAttributeValues={":vv": virus, ":fv": final, ":st": status},
    )

    for user_id in msg.get("ownerUserIds", []):
        rows = INBOX.query(KeyConditionExpression=Key("userId").eq(user_id)).get("Items", [])
        for row in rows:
